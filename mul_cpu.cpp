...
class CustomDeviceSelector {
 public:
  ...
  int operator()(const device &dev) const {
    // Give highest priority to CPU devices
    if (dev.is_cpu())
      return 3; // Highest rating for CPU
    else if (dev.is_gpu())
      return 2; // Lower rating for GPU
    else
      return 1; // Lowest rating for others (if needed)
  }
};
...
int main() {
  ...
  try {
    // Removed vendor name to allow for generic CPU selection
